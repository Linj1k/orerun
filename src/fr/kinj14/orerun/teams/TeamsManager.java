	...
	public void updateScoreBoard(Player player) {
		for(Entry<Player, ScoreboardSign> scoreb : main.scorebaordMap.entrySet()) {
			if(scoreb.getKey() != null && scoreb.getValue() != null) {				
				...
				if(scorebTeam != null) {					
					scoreb.getValue().setLine(5, scorebTeam.getTag()+"█§rPoints: 0");
				} else {
					scoreb.getValue().setLine(5, "§rPoints : 0");
				}
			}
		}
	}
		...
		if(scorebTeam != null) {
			for(Player pla : scorebTeam.getTeamPlayers()) {
				if(main.scorebaordMap.containsKey(pla)) {
					main.scorebaordMap.get(pla).setLine(5, scorebTeam.getTag()+"█§rPoints: "+(int)scorebTeam.getPoints());
				}
			}
		}
	}
